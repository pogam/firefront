...
double FRPModel::getValue(double* valueOf
	...
	if ( bt == et ){
		if ( bt < at ) return 0;
		if ( bt < at + burningDuration ) return FRP_ratio * nominalHeatFlux / FRP_max;
		return 0;
	}
	...
	if ( bt < at and et <= at + burningDuration ) return FRP_ratio * nominalHeatFlux*(et-at)/(et-bt) / FRP_max;
	/* begin time outside interval, end time outside */
	if ( bt < at and et > at + burningDuration ) return FRP_ratio * nominalHeatFlux*burningDuration/(et-bt) / FRP_max;
	/* begin time inside interval, end time inside */
	if ( bt >= at and et <= at + burningDuration ) return FRP_ratio * nominalHeatFlux / FRP_max;
	/* begin time inside interval, end time outside */
	return FRP_ratio * nominalHeatFlux*(at+burningDuration-bt)/(et-bt) / FRP_max;
